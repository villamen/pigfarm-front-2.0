    ...
    <div class="modal-overlay" v-if="mostrar">
      <div class="editar-modal">
        <div class="imagen-lateral">
          <img class="imagen" src="/editar-cerdo2.jpg" alt="Editar vacuna" />
        </div>

        <div class="formulario-edicion">
          <h2>Editar Vacuna</h2>
          <form @submit.prevent="editarVacuna">
            <label>Nombre:</label>
            <br>
            <input v-model="vacunaEdit.nombre" required />
              <br>
              <br>
    ...
    async editarVacuna() {
      ...
      try {
        const res = await fetch(`https://pigfarm-back-2-0.onrender.com/vacunas/${this.vacunaEdit.id_vacuna}`, {
          ...
          body: JSON.stringify({
            nombre: this.vacunaEdit.nombre,
            descripcion: this.vacunaEdit.descripcion,
            fecha_vencimiento: this.vacunaEdit.fecha_vencimiento
          })
        });
        ...
        if (res.ok) {
          // alert('Vacuna actualizada');
           await Swal.fire({
            icon: 'success',
            text: 'Vacuna actualizada',
            confirmButtonColor: '#3085d6'
          });
          this.$emit('actualizada');
          this.$emit('cerrar');
        } else {
          const err = await res.json();
          // alert(err.error || 'Error al actualizar vacuna');
           Swal.fire({
            icon: 'error',
            title: 'Error',
            text: data.error || 'Error al actualizar vacuna',
